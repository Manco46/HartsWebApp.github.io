...
Public Class AccountController
    ...
    Public Function Register() As ActionResult
        ...
        If User.Identity.IsAuthenticated Then
            If UserManager.IsInRole(User.Identity.GetUserId(), "OWNER") Then

                Dim results = From r In db.Roles() Select r.Name

                ViewBag.RoleComboData = results.ToList()

            ElseIf UserManager.IsInRole(User.Identity.GetUserId(), "ADMIN") Then
                Dim lstAdminRoles As New List(Of String)
                lstAdminRoles.Add("ADMIN")
                lstAdminRoles.Add("OWNER")
                Dim query = From r In db.Roles()
                            Where Not lstAdminRoles.Contains(r.Name)
                            Select r.Name
                ViewBag.RoleComboData = query.ToList()
    ...
    <AllowAnonymous>
    <ValidateAntiForgeryToken>
    Public Async Function Register(model As RegisterViewModel, Optional dropDownRole As String = "Default") As Task(Of ActionResult)
        If ModelState.IsValid Then
            ...
            If result.Succeeded Then
                If Not dropDownRole = "Default" Then
                    Await UserManager.AddToRoleAsync(user.Id, dropDownRole)
                Else
            ...
            Else
                AddErrors(result)
                ModelState.AddModelError("", "Error while creating your account, please try again later.")
            End If
        End If
